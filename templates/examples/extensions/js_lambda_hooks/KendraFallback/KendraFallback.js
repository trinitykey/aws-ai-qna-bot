  ...
  return new Promise(function(resolve, reject) {
    kendraClient.query(params, function(err, data) {
      ...
      if (err) {
        ...
        console.log(
          "Data from Kendra request:" + JSON.stringify(data, null, 2)
        );
        resArray.push(data);
        resolve(data);
      }
...
async function routeKendraRequest(event, context) {
  ...
  let indexes = event.req["_settings"]["ALT_SEARCH_KENDRA_INDEXES"]
    ? event.req["_settings"]["ALT_SEARCH_KENDRA_INDEXES"]
    : process.env.KENDRA_INDEXES;
  console.log(`Kendra indexes ${indexes}`);
  ...
  kendraIndexes.forEach(function(index, i) {
    ...
    if (
      kendraResultsCached &&
      index === kendraResultsCached.originalKendraIndexId
    ) {
  ...
  let topAnswerMessage =
    event.req["_settings"]["ALT_SEARCH_KENDRA_TOP_ANSWER_MESSAGE"] + "\n\n"; //"Amazon Kendra suggested answer. \n\n ";
  let topAnswerMessageMd = `*${
    event.req["_settings"]["ALT_SEARCH_KENDRA_TOP_ANSWER_MESSAGE"]
  }* \n `;
  let answerMessage =
    event.req["_settings"]["ALT_SEARCH_KENDRA_ANSWER_MESSAGE"];
  ...
  let maxDocumentCount = _.get(
    event.req,
    "_settings.ALT_SEARCH_KENDRA_MAX_DOCUMENT_COUNT",
    2
  );
  var seenTop = false;
  ...
  let signS3Urls = _.get(
    event.req,
    "_settings.ALT_SEARCH_KENDRA_S3_SIGNED_URLS",
    true
  );
  let expireSeconds = _.get(
    event.req,
    "_settings.ALT_SEARCH_KENDRA_S3_SIGNED_URL_EXPIRE_SECS",
    300
  );
    ...
    if (res && res.ResultItems.length > 0) {
      res.ResultItems.forEach(function(element, i) {
        ...
        if (
          ...
          answerMessage +=
            "\n\n " +
            element.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text.replace(
              /\r?\n|\r/g,
              " "
            );

          // Emboldens the highlighted phrases returned by the Kendra response API in markdown format
          let answerTextMd = element.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text.replace(
            /\r?\n|\r/g,
            " "
          );
          // iterates over the answer highlights in sorted order of BeginOffset, merges the overlapping intervals
          var sorted_highlights = mergeIntervals(
            element.AdditionalAttributes[0].Value.TextWithHighlightsValue
              .Highlights
          );
          var j, elem;
          for (j = 0; j < sorted_highlights.length; j++) {
            ...
            let offset = 4 * j;
            let beginning = answerTextMd.substring(
              0,
              elem.BeginOffset + offset
            );
            let highlight = answerTextMd.substring(
              elem.BeginOffset + offset,
              elem.EndOffset + offset
            );
          ...
          if (seenTop == false) {
            ...
            let answerText = element.AdditionalAttributes[0].Value.TextWithHighlightsValue.Text.replace(
              /\r?\n|\r/g,
              " "
            );
          ...
          answerMessage =
            faqanswerMessage +
            "\n\n " +
            element.AdditionalAttributes[1].Value.TextWithHighlightsValue.Text.replace(
              /\r?\n|\r/g,
              " "
            );
          ...
          answerDocumentUris = [];
          let answerTextMd = element.AdditionalAttributes[1].Value.TextWithHighlightsValue.Text.replace(
            /\r?\n|\r/g,
            " "
          );
          // iterates over the FAQ answer highlights in sorted order of BeginOffset, merges the overlapping intervals
          var sorted_highlights = mergeIntervals(
            element.AdditionalAttributes[1].Value.TextWithHighlightsValue
              .Highlights
          );
          var j, elem;
          for (j = 0; j < sorted_highlights.length; j++) {
          ...
          element.DocumentExcerpt.Text &&
          element.DocumentURI
        ) {
          const docInfo = {};
          // if topAnswer found, then do not show document excerpts
          if (seenTop == false) {
            docInfo.text = element.DocumentExcerpt.Text.replace(
              /\r?\n|\r/g,
              " "
            );
            // iterates over the document excerpt highlights in sorted order of BeginOffset, merges overlapping intervals
            var sorted_highlights = mergeIntervals(
              element.DocumentExcerpt.Highlights
            );
            var j, elem;
            for (j = 0; j < sorted_highlights.length; j++) {
              elem = sorted_highlights[j];
              let offset = 4 * j;
              let beginning = docInfo.text.substring(
                0,
                elem.BeginOffset + offset
              );
              let highlight = docInfo.text.substring(
                elem.BeginOffset + offset,
                elem.EndOffset + offset
              );
            ...
            if (foundAnswerCount == 0 && foundDocumentCount == 0) {
              speechMessage = element.DocumentExcerpt.Text.replace(
                /\r?\n|\r/g,
                " "
              );
              if (sorted_highlights.length > 0) {
                var highlight = speechMessage.substring(
                  sorted_highlights[0].BeginOffset,
                  sorted_highlights[0].EndOffset
                );
  ...
  if (foundAnswerCount > 0 || foundDocumentCount > 0) {
    ...
    var ssmlMessage = `${answerMessage
      .substring(0, 600)
      .replace(/\r?\n|\r/g, " ")}`;
    if (speechMessage != "") {
      ssmlMessage = `${speechMessage
        .substring(0, 600)
        .replace(/\r?\n|\r/g, " ")}`;
    }
  ...
  console.log("Do we need translation for " + event.res.meessage + "?" )
  ...
  if (answerDocumentUris.size > 0) {
    ...
    answerDocumentUris.forEach(function(element) {
      ...
      event.res.session.appContext.altMessages.markdown += `[${label}](${
        element.uri ? element.uri : element.Uri
      })`;
    });
  }
  ...
  if (seenTop == false) {
    helpfulDocumentsUris.forEach(function(element) {
      // If top answer is already included, subtract that when considering maxDocumentCount.
      //This corrects a previous defect where if "maxDocumentCount" was 1 and no "top" answers were returned
      //It wouldn't return any results, just the header.
...

  {
    ...
    if (_.get(event.req._settings, "ENABLE_MULTI_LANGUAGE_SUPPORT")) {
        ...
        usrLang = _.get(event.req, "session.userLocale");
      if (usrLang != "en") {
  ...
  if (kendraQueryId) {
    _.set(
      event,
      "res.session.qnabotcontext.kendra.kendraQueryId",
      kendraQueryId
    );
    _.set(
      event,
      "res.session.qnabotcontext.kendra.kendraIndexId",
      kendraIndexId
    );
    _.set(
      event,
      "res.session.qnabotcontext.kendra.kendraResultId",
      kendraResultId
    );
    if (event.res.result) {
      _.set(
        event,
        "res.session.qnabotcontext.kendra.kendraResponsibleQid",
        event.res.result.qid
      );
    }
  }
